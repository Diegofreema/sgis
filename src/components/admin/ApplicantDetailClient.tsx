...
export function ApplicantDetailClient({ application, supportingDocuments, examAttempt }: Props) {
  ...
  const [submitting, setSubmitting] = useState<"pending" | "approved" | "rejected" | null>(null);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 print:hidden">
        <div className="flex items-center gap-3 min-w-0">
          <Button variant="ghost" size="icon" onClick={() => navigate({ to: "/admin/applicants" })}>
            <ArrowLeft className="h-4 w-4" />
          </Button>
          <div>
            <h1 className="font-serif text-xl font-semibold text-foreground">{fullName}</h1>
            <p className="text-sm text-muted-foreground">{application.applicationCode}</p>
          </div>
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="lg:col-span-2 space-y-6">
            ...
            <CardContent className="space-y-5">
              <div className="grid gap-4">
                <dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                  ...
                  <Info label="Application ID" value={application.applicationCode} />
                  ...
                  <Info label="Intended Class" value={application.intendedClass} />
                  <Info label="Previous School" value={application.previousSchool ?? "Not provided"} />
                  <Info label="Address" value={application.address} wide />
                  <Info
                    label="State / LGA"
                    value={
                      [application.state, application.lga].filter(Boolean).join(" / ") ||
                      "Not provided"
                    }
                  />
                  <Info
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-serif text-base">Guardian and Documents</CardTitle>
            </CardHeader>
            <CardContent className="space-y-5">
              <dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                <Info label="Guardian Name" value={application.guardianName ?? "Not provided"} />
                <Info label="Guardian Phone" value={application.guardianPhone ?? "Not provided"} />
                <Info label="Guardian Email" value={application.guardianEmail ?? "Not provided"} />
              </dl>
              <div className="space-y-3">
                {application.receiptUrl && (
                  <Button asChild variant="outline" size="sm">
                    <a href={application.receiptUrl} target="_blank" rel="noopener noreferrer">
          ...
          <Card>
            <CardHeader>
              <CardTitle className="font-serif text-base">Exam Outcome</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {!examAttempt ? (
                <p className="text-sm text-muted-foreground">No exam attempt recorded yet.</p>
              ) : (
                <dl className="grid grid-cols-2 gap-x-6 gap-y-4 text-sm">
                  ...
                  <Info
                    label="Submitted"
                    value={
                      examAttempt.attempt.submittedAt
                        ? formatDate(examAttempt.attempt.submittedAt)
                        : "Not submitted"
                    }
                  />
                  ...
                  <Info
                    ...
                    value={
                      examAttempt.attempt.score !== null && (examAttempt.attempt.totalMarks ?? 0) > 0
                        ? `${Math.round(
                            (Number(examAttempt.attempt.score) /
                              Number(examAttempt.attempt.totalMarks)) *
                              100,
                          )}%`
                        : "—"
                    }
        ...
        <div className="space-y-4 print:hidden">
          <Card>
            <CardHeader>
              <CardTitle className="font-serif text-base">Review Decision</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
      ...
      <AlertDialog open={deleteOpen} onOpenChange={setDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            ...
            <AlertDialogDescription>
              This permanently deletes {fullName}&rsquo;s application ({application.applicationCode})
              and its uploaded documents. This cannot be undone.
            </AlertDialogDescription>
          </AlertDialogHeader>
          <AlertDialogFooter>
